...
public class AnnouncementService {
        ...
        if (Constants.ROLE_SUPPLIER.equals(role)) {
            ...
            wrapper.and(w -> w.eq(Announcement::getVisibilityLevel, Constants.VISIBILITY_PUBLIC)
                    .or(ow -> ow.eq(Announcement::getVisibilityLevel, Constants.VISIBILITY_RESTRICTED)
                            .and(iw -> iw.apply("JSON_CONTAINS(visible_supplier_ids, CAST({0} AS JSON))",
                                    supplierId))));
        }
    ...
    public void updateAnnouncement(Announcement announcement, Long currentUserId, String currentUserRole) {
        log.info("[AnnouncementService] 开始更新公告, ID: {}, 当前用户ID: {}, 角色: {}", announcement.getId(), currentUserId,
                currentUserRole);
        ...
        log.info("[AnnouncementService] 权限校验: Constants.ROLE_ADMIN='{}', currentUserRole='{}', trimmedRole='{}'",
                Constants.ROLE_ADMIN, currentUserRole, role);
        if (!Constants.ROLE_ADMIN.equalsIgnoreCase(role)) {
            ...
            if (existingAnnouncement.getPublisherId() == null
                    || !existingAnnouncement.getPublisherId().equals(currentUserId)) {
        ...
        if (Constants.ANNOUNCEMENT_STATUS_PUBLISHED.equals(existingAnnouncement.getStatus())
            ...
            if (!existingAnnouncement.getTitle().equals(announcement.getTitle())
                    || !existingAnnouncement.getContent().equals(announcement.getContent())) {
