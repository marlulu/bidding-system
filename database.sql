/*
 内部招标网站系统数据库脚本 (最终完整版)
 修复了所有业务模块字段不一致的问题，确保与后端实体类完全同步。
*/
...
CREATE TABLE `bid_announcement` (
  ...
  `contact_person` varchar(50) DEFAULT NULL, -- 补全字段
  `contact_phone` varchar(20) DEFAULT NULL, -- 补全字段
  `attachment_files` text, -- 补全字段
  `visibility_level` varchar(20) DEFAULT 'PUBLIC', -- 补全字段
  `visible_supplier_ids` text, -- 补全字段
  `status` varchar(20) DEFAULT 'PUBLISHED',
  `publisher_id` bigint(20) DEFAULT NULL, -- 补全字段
...
CREATE TABLE `policy_regulation` (
  ...
  `attachment_files` text, -- 补全字段
  `publisher_id` bigint(20) DEFAULT NULL, -- 补全字段
  `publish_time` datetime DEFAULT NULL, -- 补全字段
  `status` varchar(20) DEFAULT 'PUBLISHED', -- 补全字段
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `system_notice` (
  ...
  `target_type` varchar(20) DEFAULT 'ALL', -- 补全字段
  `target_supplier_ids` text, -- 补全字段
  `publisher_id` bigint(20) DEFAULT NULL, -- 补全字段
  `publish_time` datetime DEFAULT NULL, -- 补全字段
  `status` varchar(20) DEFAULT 'PUBLISHED', -- 补全字段
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
